...
4. Add or update the implemented chapter page and route.
   ...
   - When a mobile chapter playback track has many event dots, make the dot text readable by giving each dot enough stable horizontal space, using `letter-spacing: 0`, and adding a high-contrast label background if labels are shown under the dots. Do not let dot labels overlap or depend on viewport-width font scaling.
